...
PATCH="$ROOT/patches/box3d-flat-patch-ghost-contact.patch"
...
if git -C "$DEST" apply --reverse --check "$PATCH" >/dev/null 2>&1; then
  echo "box3d flat-patch ghost-contact patch already applied"
elif git -C "$DEST" apply --check "$PATCH"; then
  git -C "$DEST" apply "$PATCH"
  echo "box3d flat-patch ghost-contact patch applied"
else
  echo "box3d flat-patch ghost-contact patch does not apply cleanly" >&2
  exit 1
fi
