...
export class Body {
    ...
    createBox(opts) {
        ...
        return new Shape(B.shapeBox(this.handle, opts.halfExtents.x, opts.halfExtents.y, opts.halfExtents.z, d, f, r));
    }
    createSphere(opts) {
        const d = opts.density === undefined ? -1 : opts.density;
        const f = opts.friction === undefined ? -1 : opts.friction;
    ...
    applyLinearImpulseToCenter(v, wake) {
        B.bodyImpulse(this.handle, v.x, v.y, v.z, wake ? 1 : 0);
    }
    destroy() { B.bodyDestroy(this.handle); }
}
export class World {
    handle;
    constructor(opts) {
    ...
    explode(opts) {
        ...
        B.worldExplode(this.handle, opts.position.x, opts.position.y, opts.position.z, opts.radius, falloff, ipa);
    }
    destroy() { B.worldDestroy(this.handle); }
}
